...
async function loadMyRequests(){
  ...
  if(!data?.length){box.innerHTML='<div class="ride"><b>No ride requests yet.</b><small>Find a ride and request a seat.</small></div>';await loadPaymentHistory();return;}
  const payments=await getPaymentForRequests(data.map(x=>x.id));
  box.innerHTML=data.map(r=>{const ride=r.rides||{};const amount=Number(ride.price ?? ride.contribution ?? 0);const payment=payments[r.id];const paid=payment?.status==='paid';const cashPending=payment?.status==='cash_pending';const payBtn=r.status==='accepted'&&amount>0&&!paid&&!cashPending?`<button class="payment-mini-btn" onclick="openPayment('${r.id}')">📱 Pay ₹${escapeHtml(amount)}</button>`:'';const booking=paid?`<span class="booking-confirmed">✓ Booking confirmed · ${payment?.payment_method==='cash'?'Cash paid':'UPI paid'}</span>`:cashPending?'<span class="booking-pending payment-cash-badge">💵 Cash selected · Pay driver</span>':(r.status==='accepted'&&amount>0?'<span class="booking-pending">Payment pending</span>':'');return `<div class="ride"><b>${escapeHtml(ride.from_location||'')} → ${escapeHtml(ride.to_location||'')}</b><small>${escapeHtml(formatDate(ride.ride_date))} · ${escapeHtml(formatTime(ride.ride_time))}${amount>0?' · ₹'+escapeHtml(amount):' · Free'}</small><span>Status: ${escapeHtml(r.status||'pending')}</span><div class="request-actions">${payBtn}${booking}</div></div>`}).join('');
...
async function loadMyRides(){
  ...
  if(myRidesTab==='driver'){
    ...
    box.innerHTML=data.map(r=>{
      ...
      return `<div class="ride" data-ride-id="${escapeHtml(r.id)}"><b>${escapeHtml(r.from_location||r.from_place||'')} → ${escapeHtml(r.to_location||r.to_place||'')}</b><span class="status-badge status-${escapeHtml(status)}">${escapeHtml(rideStatusLabel(status))}</span>${rideDetailMarkup(r)}<div class="ride-actions"><button onclick="viewRideDetails('${r.id}','driver')">View details</button>${start}${complete}${cancel?`<button class="danger" onclick="cancelRide('${r.id}')">Cancel ride</button>`:''}</div></div>`;
    }).join('');
  } else {
    ...
    box.innerHTML=joined.map(r=>{const ride=r.rides||{};return `<div class="ride"><b>${escapeHtml(ride.from_location||ride.from_place||'')} → ${escapeHtml(ride.to_location||ride.to_place||'')}</b><span class="status-badge status-${escapeHtml(String(ride.status||'open').toLowerCase())}">${escapeHtml(rideStatusLabel(ride.status||'open'))}</span>${rideDetailMarkup(ride,`<div class="muted" style="margin-top:8px">Driver: <strong>${escapeHtml(ride.profiles?.full_name||'BIKUBOO rider')}</strong></div>`)}<div class="ride-actions"><button onclick="viewRideDetails('${r.ride_id}','passenger')">View details</button>${!['started','completed','cancelled'].includes(String(ride.status||'open').toLowerCase())?`<button class="danger" onclick="cancelRideRequest('${r.id}')">Cancel request</button>`:''}${ride.status==='started'?'<span class="booking-confirmed">🏍️ Ride in progress</span>':''}${ride.status==='completed'?'<span class="booking-confirmed">✓ Ride completed</span>':''}${new Date(`${ride.ride_date}T${ride.ride_time||'00:00:00'}`)<Date.now()?`<button class="secondary" onclick="openRating('${r.ride_id}','${ride.driver_id}','Rate ${escapeHtml(ride.profiles?.full_name||'your driver')}')">⭐ Rate driver</button>`:''}</div></div>`;}).join('');
  }
}
